/**
 * Stack Sequence Vertically (Sum Heights)
 * 
 * Changes the active sequence's dimensions to accommodate stacking selected clips vertically.
 * - Width: Maximum width among all selected clips
 * - Height: Sum of all selected clips' heights
 * 
 * Example: Two images (1000×1000, 1200×1500) → Sequence becomes 1200×2500
 * 
 * Features:
 * - Works with currently selected clips in the timeline
 * - Sums heights for vertical stacking
 * - Uses maximum width to fit all clips
 */
...
/**
 * Calculates dimensions for vertical stacking
 * @param {Array} clips - Array of clip dimension objects
 * @returns {Object} Object with width (max) and height (sum)
 */
function calculateVerticalStackDimensions(clips) {
    var maxWidth = 0;
    var totalHeight = 0;
    
    for (var i = 0; i < clips.length; i++) {
        if (clips[i].width > maxWidth) {
            maxWidth = clips[i].width;
        }
        totalHeight += clips[i].height;
    }
 ...
 */
function stackSequenceVertically() {
    ...
    var stackedDimensions = calculateVerticalStackDimensions(clipDimensions);
